...
| プラン | SSM に設定する値 | 使用 API |
|--------|----------------|---------|
| **Free** | ダッシュボードの「リフレッシュトークン」 | v1 API (`Client`) |
| Light / Standard / Premium | ダッシュボードの「API キー」 | v2 API (`ClientV2`) |

**Free プランの注意点**:
- バッチ処理の対象日は自動的に「今日 − 12週」に調整されます
- バックフィルは最大2年分に制限されます
- APIコール間に12秒のスリープが挿入されます (バックフィルは約6分かかります)
