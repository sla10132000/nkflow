...
class TestSaveRaw:
    def test_creates_json_in_s3(self, s3_bucket):
        ...
        assert key == "raw/market/equity/jquants/daily_prices/2026-03-06.json"
        stored = _get_raw(s3_bucket, key)
        assert stored["category"] == "market"
        assert stored["subcategory"] == "equity"
    ...
    def test_skips_existing_file(self, s3_bucket):
        data1 = [{"v": 1}]
        data2 = [{"v": 2}]
        save_raw("cat", "sub", "src", "type", "2026-01-01", data1)
        save_raw("cat", "sub", "src", "type", "2026-01-01", data2)

        stored = _get_raw(s3_bucket, "raw/cat/sub/src/type/2026-01-01.json")
        assert stored["data"] == data1  # 上書きされていない

    def test_overwrite_flag(self, s3_bucket):
        save_raw("cat", "sub", "src", "type", "2026-01-01", [{"v": 1}])
        save_raw("cat", "sub", "src", "type", "2026-01-01", [{"v": 2}], overwrite=True)
    ...
    def test_handles_dataframe(self, s3_bucket):
        ...
        key = save_raw("market", "equity", "jquants", "stock_master", "2026-03-06", df)
    ...
    def test_handles_non_serializable_types(self, s3_bucket):
        ...
        key = save_raw("cat", "sub", "test", "misc", "2026-03-06", data)
    ...
    def test_failure_returns_none(self, monkeypatch):
        ...
        result = save_raw("cat", "sub", "src", "type", "2026-01-01", {"x": 1})
