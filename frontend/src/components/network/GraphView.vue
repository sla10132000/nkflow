...
function buildNetwork() {
  ...
  const nodes = new DataSet(
    props.data.nodes.map(n => {
      ...
      const size = props.anchorMode && inflow[n.id]
        ...
        : baseSize
      const isHotNode = props.anchorMode && inflow[n.id] && inflow[n.id] / maxInflow > 0.5
      return {
        id: n.id,
        label: n.id,
        title: props.anchorMode && inflow[n.id]
          ? `${n.id}\n${n.label}\n${n.group}\n流入: ${inflow[n.id]}日`
          : `${n.id}\n${n.label}\n${n.group}`,
        color: {
  ...
  const maxEdgeCount = Math.max(1, ...props.data.edges.map(e => e.edge_count || 0))

  const edges = new DataSet(
    props.data.edges.map((e, i) => {
      const hasCount = e.edge_count != null
      const count = e.edge_count || 0
      const width = props.anchorMode && hasCount
