...
class TestBatchHandler:
    def _patch_all(self, n_fetched: int = 5):
        return [
            ...
            patch("src.batch.td_sequential.compute_td_sequential", return_value=0),      # Phase 22
            patch("src.batch.compute.generate_overview_snapshot", return_value={}),      # 概要スナップショット
        ]
    ...
    def test_returns_200_on_trading_day(self, tmp_path, monkeypatch):
        ...
        with patches[0], patches[1], patches[2], patches[3] as mock_fetch, \
             ...
             patches[6] as mock_graph, patches[7], patches[8], patches[9], patches[10], patches[11], patches[12], patches[13]:
