...
from src.api.routers import backtest, commodities, fear_indices, forex, investor_flows, margin, network, news, overview, portfolio, prices, sector_rotation, stock, summary, supercycle, td_sequential, us_indices, ytd_highs
...
app.include_router(investor_flows.router, prefix="/api")   # Phase 23 投資主体別フロー
app.include_router(commodities.router, prefix="/api")      # Phase 26 コモディティ
app.include_router(supercycle.router, prefix="/api")       # Phase 27 スーパーサイクル分析
app.include_router(overview.router, prefix="/api")         # 概要ページスナップショット
